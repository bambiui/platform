...
function inlinePrimitiveSource(source) {
...
// Shared helper identifiers that are extracted into bambi-helpers.ts instead of
// being inlined.  Order matters: it determines the import statement order.
...
function extractControllerBehavior(source, componentName) {
  ...
  // Remove BambiController interface — it is imported as BambiBehavior from
  // bambi-helpers.ts instead of being inlined.
  if (sf.getInterface("BambiController")) {
    sf.getInterface("BambiController").remove();
    usedHelpers.push("BambiBehavior");
  }

  // Remove known shared DOM helper functions.
