  ...
  return getEmbeddedChildrenForPart(part, contract, options)
    .map(({ embedded, child }) => {
      const attrs = (embedded.attributes ?? [])
        .map((attribute) => vueEmbeddedAttributeLine(attribute))
        .join("");
...
const isSelected = computed(() => selectedValue?.value === props.${ssrState.valuePropName});`
  ...
  const ssrAttrs =
    ssrPart && valueHandling
      ? ssrPart.attributes
          .map((attribute) => vueSsrAttributeLine(attribute))
          .join("")
      : "";
  ...
  const propsDecl = propTypeParts ? `const props = defineProps<Props>();` : "";
...
function vueRootFile({
  ...
  const propsTypeName = hasEventCallbacks
  ...
  const ssrSelectedValueLine = ssrState
    ? `const selectedValue = computed(() => ${ssrSelectedExpression});
  ...
  const rootAttrLines = contract.props
    ...
    .map((prop) => {
      ...
      if (prop.type === "boolean")
        return `    :${prop.attribute}="props.${prop.name} ? 'true' : undefined"`;
...
`
  ...
  const eventAbortBlock = hasEventCallbacks
  ...
  const polymorphicState = polymorphicRootPropName
    ? `const componentTag = computed(() => props.${polymorphicRootPropName} ?? "${root.element}");
...
`
    : "";
  const rootTemplate = polymorphicRootPropName
    ? `<component
    :is="componentTag"
    ref="rootRef"
    v-bind="$attrs"
...
defineOptions({ inheritAttrs: false });
import { ref, computed, watch, onMounted, onUnmounted${ssrState ? ", provide" : ""} } from "vue";
${helperImportLine}
${publicContractSource}
${primitivesBlock ? `\n${primitivesBlock}\n` : ""}
${behaviorSource}
...
export function createVueArtifact(options) {
  ...
  for (const part of contract.parts.filter(
    (p) => p.name !== "root" && !omittedParts.has(p.name),
  )) {
