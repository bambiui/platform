...
/>`}
  ...
  <TabItem label="Svelte">
    <Code
      lang="svelte"
      code={`{#snippet header()}My App{/snippet}
{#snippet footer()}<button type="button">Log out</button>{/snippet}
...
>
  <template #header>My App</template>
  <template #footer><button type="button">Log out</button></template>
</Sidebar>`}
    />
  </TabItem>
  <TabItem label="Astro">
...
>
  ...
  <button slot="footer" type="button">Log out</button>
</Sidebar>`}
    />
  </TabItem>
</Tabs>
