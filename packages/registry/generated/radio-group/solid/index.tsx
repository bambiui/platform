...
import { createContext, createEffect, onMount, onCleanup, splitProps, children, useContext, type JSX } from "solid-js";
...
class RadioGroupBehavior implements BambiBehavior {
  ...
  constructor(root: Element, options: RadioGroupOptions = {}) {
    ...
    this._id = root.id || `bambi-radio-group-${++RadioGroupBehavior._idCounter}`;
...
export interface RadioGroupProps extends Omit<RadioGroupOptions, "controlled">, Omit<JSX.IntrinsicElements["div"], keyof Omit<RadioGroupOptions, "controlled">> {
...
export function RadioGroup(props: RadioGroupProps) {
  const [local, rest] = splitProps(props, ["value", "defaultValue", "name", "orientation", "loop", "disabled", "required", "invalid", "ariaLabel", "ariaLabelledby", "ariaDescribedby", "ariaErrormessage", "onValueChange", "children"]);
  ...
  onMount(() => {
    ...
    rootRef!.addEventListener(RADIO_GROUP_EVENT_VALUE_CHANGE, onValueChangeHandler);
    ...
    behavior.sync();
    onCleanup(() => {
      rootRef!.removeEventListener(RADIO_GROUP_EVENT_VALUE_CHANGE, onValueChangeHandler);
      behavior?.destroy();
    });
  });
...
export function RadioGroupIndicator(props: RadioGroupIndicatorProps) {
  ...
  return (
    <span
      {...rest}
      data-bambi-radio-group-indicator=""
    >
      {local.children}
    </span>
  );
