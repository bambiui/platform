...
import { ref, computed, watch, onMounted, onUnmounted, onUpdated, provide } from "vue";
...
interface RadioGroupProps extends Omit<RadioGroupOptions, "controlled"> {
...
const selectedValue = computed(() => props.value ?? props.defaultValue);
provide("bambi-radio-group-value", selectedValue);
...
onMounted(() => {
  rootRef.value!.addEventListener(RADIO_GROUP_EVENT_VALUE_CHANGE, (event: Event) => {
    const e = event as CustomEvent<RadioGroupValueChangeDetail>;
    props.onValueChange?.(e.detail);
  }, { signal: eventAbort.signal });
  behavior = new RadioGroupBehavior(rootRef.value!, {
      value: props.value,
...
watch(
  () => ({ value: props.value, defaultValue: props.defaultValue, name: props.name, orientation: props.orientation, loop: props.loop, disabled: props.disabled, required: props.required, invalid: props.invalid, ariaLabel: props.ariaLabel, ariaLabelledby: props.ariaLabelledby, ariaDescribedby: props.ariaDescribedby, ariaErrormessage: props.ariaErrormessage }),
  () => {
    behavior?.update?.({
      value: props.value,
      defaultValue: props.defaultValue,
