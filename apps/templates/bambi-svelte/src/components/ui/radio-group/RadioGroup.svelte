...
interface Props extends Omit<RadioGroupOptions, "controlled"> {
  children?: Snippet;
  class?: string;
  [key: string]: unknown;
...
setContext("bambi-radio-group-value", () => value ?? defaultValue);
...
onMount(() => {
  ...
  rootEl!.addEventListener(RADIO_GROUP_EVENT_VALUE_CHANGE, onValueChangeHandler);
  behavior = new RadioGroupBehavior(rootEl!, {
      value,
      defaultValue,
      name,
      orientation,
  ...
  behavior.sync();
  return () => {
    rootEl!.removeEventListener(RADIO_GROUP_EVENT_VALUE_CHANGE, onValueChangeHandler);
    behavior?.destroy();
  };
});
...
// changes at runtime, wrap <RadioGroup> in a {#key} block keyed
// to the structure.
$effect(() => {
  behavior?.update?.({
      value,
